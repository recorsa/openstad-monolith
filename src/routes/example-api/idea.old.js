...
// Edit idea
// ---------
router.route('/old/:ideaId(\\d+)/edit')
...
// Delete idea
// -----------
router.route('/old/:ideaId(\\d+)/delete')
...
// is called.
router.route('/old/:ideaId(\\d+)/vote')
	.all(fetchIdea())
...
(function() {
	// New argument.
	router.route('/old/:ideaId(\\d+)/arg/new')
		.all(fetchIdea())
	...
	// Reply to argument.
	router.route('/old/:ideaId(\\d+)/arg/reply')
		.all(fetchIdea())
		.all(fetchArgument)
...
// Edit argument
// -------------
router.route('/old/:ideaId(\\d+)/arg/:argId/edit')
	.all(fetchIdea())
	.all(fetchArgument)
...
// Delete argument
// ---------------
router.route('/old/:ideaId(\\d+)/arg/:argId/delete')
	.all(fetchIdea())
	.all(fetchArgument)
	...
	.delete(function( req, res, next ) {
		...
		var isReaction = !!argument.parentId; // dit onderscheid bestaat alleen in stemvan maar zou niet uit moeten maken
		argument.destroy()
			.then(function() {
...
// Vote for argument
// -----------------
router.route('/old/:ideaId(\\d+)/arg/:argId(\\d+)/vote')
	.all(fetchIdea())
	.all(fetchArgument)
...
// -------------------------
router.route('/old/:ideaId(\\d+)/status')
...
// xxx alleen in stemvan
// --------------------------------
router.route('/old/:ideaId(\\d+)/meeting')
	.all(fetchIdea())
	.all(auth.can('idea:admin'))
	.put(function( req, res, next ) {
		var idea = req.idea;
...
// -----------------------
router.route('/old/:ideaId(\\d+)/mod_break')
	.all(fetchIdea())
	.all(auth.can('idea:admin'))
	.get(function( req, res, next ) {
...
// Admin: view votes
// -----------------
router.route('/old/:ideaId(\\d+)/votes')
...
// -------------------------
router.route('/old/:ideaId(\\d+)/vote/:voteId/toggle_checked')
	.all(fetchVote)
	.all(auth.can('idea:admin'))
	.get(function( req, res, next ) {
		var ideaId = req.params.ideaId;
...
// DEZE MOET NOG VOOR API
	let target = process.env.NODE_ENV == 'stemtool' ? db.PollVote : db.Vote;
	target.findById(voteId)
		.then(function( vote ) {
			if( vote ) {
				req.vote = vote;
			}

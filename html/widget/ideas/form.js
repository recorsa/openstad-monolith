class EditIdeaWidget extends HTMLElement {
	...
	connectedCallback () {
		...
		element.src = "{{widgetUrl}}/example-widget/textarea-with-counter?access_token=VRIth7Tv1j1tEyQ7Z8TnhSaqnmDXFenXoYCxrjxKMO9QwZYgLEiRfM1IU48zfMCxJEcNBm88HIzznomBhYgC3IRVFs9XguP3vi40";
		element.onload = self.initSummary.bind(self);
		self.shadowRoot.appendChild(element);

		// load textarea-with-counter
		let element2 = document.createElement('script');
		element2.src = "{{widgetUrl}}/example-widget/openstad-map?access_token=VRIth7Tv1j1tEyQ7Z8TnhSaqnmDXFenXoYCxrjxKMO9QwZYgLEiRfM1IU48zfMCxJEcNBm88HIzznomBhYgC3IRVFs9XguP3vi40";
	...
	fetch() {
		...
		// TODO: CORS
		let url = `{{apiUrl}}/example-api/site/{{siteId}}/idea/${self.getAttribute('data-id')}`
	...
	submit() {
		...
		let url = '{{apiUrl}}/example-api/site/{{siteId}}/idea/';
		let method = 'post';
